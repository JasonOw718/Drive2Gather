  ...
  <div class="min-h-screen flex flex-col justify-between items-center bg-white px-4 pt-[max(2.5rem,env(safe-area-inset-top))] pb-[max(env(safe-area-inset-bottom),2.5rem)]" style="max-width: 420px; margin: 0 auto; font-family: 'Roboto', sans-serif;">
    ...
    <div class="w-full flex flex-col items-center mt-8 mb-2">
      <div class="text-2xl font-medium text-left" style="font-family: 'Roboto', sans-serif; color: #C77DFF;">
        Thanks for riding with Ride2Gather - enjoy the ride!
      </div>
      <div class="text-base mt-2 text-left" style="font-family: 'Poppins', sans-serif; color: #8C8C8C;">
        Your journey has begun. Relax and ride together.
      </div>
    </div>
    ...
    <div class="w-full text-right">
      <router-link :to="{ name: 'ReportPsg' }" class="ml-2 text-sm font-medium underline" style="font-family: 'Poppins', sans-serif; color: #C77DFF;">Report this driver??</router-link>
    </div>
    ...
    <button
      ...
      :class="isDropoff ? 'bg-[#C77DFF] text-white hover:bg-opacity-90 cursor-pointer' : 'bg-gray-300 text-gray-400 cursor-not-allowed'"
      :disabled="!isDropoff"
    ...
    <!-- Toggle for Demo (Dropoff Simulation) -->
    <div class="w-full flex flex-row items-center justify-center mt-2">
      <label class="flex items-center cursor-pointer">
        <div class="relative">
          <input type="checkbox" v-model="isDropoff" class="sr-only" />
          <div class="block bg-gray-300 w-14 h-8 rounded-full"></div>
          <div
            class="dot absolute left-1 top-1 bg-white w-6 h-6 rounded-full transition"
            :class="isDropoff ? 'translate-x-6 bg-[#C77DFF]' : ''"
          ></div>
        </div>
        <span class="ml-3 text-sm font-medium" style="font-family: 'Poppins', sans-serif; color: #C77DFF;">Simulate Dropoff (Demo)</span>
      </label>
    </div>
  </div>
</template>
