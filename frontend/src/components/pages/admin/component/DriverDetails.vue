<template>
  <div v-if="driver" class="max-w-5xl mx-auto mt-10">
    <!-- Driver Info Card -->
    <div
      class="bg-white rounded-2xl shadow-lg p-8 grid grid-cols-1 md:grid-cols-3 gap-8 items-center"
    >
      <!-- Left: Info -->
      <div class="md:col-span-2 space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            >
              {{ driver.carType }}
              <span class="text-gray-500">({{ driver.carColor }})</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-col items-center justify-center">
        <label class="block text-[#C77DFF] font-semibold mb-2"
          >Profile Picture</label
        >
        <img
          src="@/assets/images/image.png"
          alt="avatar"
          class="rounded-full border-4 border-[#C77DFF] w-32 h-32 object-cover"
        />
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg p-8 mt-8">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        <div
          v-for="(img, idx) in driver.licenseImages"
          :key="'license-' + idx"
        ...
        >
          <img
            ...
            alt="License Photo"
            class="w-48 h-32 object-cover rounded-xl border-2 border-[#C77DFF]"
          />
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg p-8 mt-8">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
        <div
          v-for="(img, idx) in driver.carPhotos"
          :key="'car-' + idx"
          class="flex justify-center"
        >
          <img
            src="@/assets/images/carphoto.jpg"
            alt="Car Photo"
            class="w-36 h-24 object-cover rounded-xl border-2 border-[#C77DFF]"
          />
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-row-reverse gap-4 mt-8">
      ...
      <button
        class="border-2 border-[#C77DFF] text-[#C77DFF] bg-white px-8 py-2 rounded-lg font-semibold hover:bg-[#F8F8F8] transition"
        @click="rejectDriver"
      >
        Reject
      </button>
...
const driver = computed(() =>
  driverStore.drivers.find((d) => d.id === driverId)
);

function approveDriver() {
  driverStore.approveDriver(driverId);
  toastStore.success(
    `Driver ${driver.value.name} has been approved successfully`
  );
